...
class DetailsLivingHabits extends React.Component {
  ...
  componentWillMount() {
    ...
    $.ajax({
      url: "/api/v1/livinghabits/getTrashed",
      dataType: "json",
      method: "GET",
      success:function(response){
        if (response != "") {
                  ...
                  <tbody>
                    {
                      this.state.list.map(function(data){
                        return (
                          <tr key={data.id}>
                            <td>{(data.do_you_go_to_day_centers_clubs_or_social_groups_2_11 == "1") ? 'SIM' : 'NÃO'}</td>
                            <td>{(data.do_you_do_any_volunteer_work_2_11 == "1") ? 'SIM' : 'NÃO'}</td>
                            <td>{(data.do_you_have_any_leisure_activities_2_11 == "1") ? 'SIM' : 'NÃO'}</td>
                            <td>{(data.do_you_engage_type_ph_act_least_three_tm_week_2_11 == "1") ? 'SIM' : 'NÃO'}</td>
                            <td>{(data.would_you_like_start_some_phy_activity_program_2_11 == "1") ? 'SIM' : 'NÃO'}</td>
                            <td>{(data.do_you_make_at_least_three_meals_a_day_2_11 == "1") ? 'SIM' : 'NÃO'}</td>
                            <td>{(data.do_you_eat_fruits_veg_your_meals_thro_the_day_2_11 == "1") ? 'SIM' : 'NÃO'}</td>
                            <td>{(data.in_at_least_one_meal_day_you_eat_meat_fish_eggs_2_11 == "1") ? 'SIM' : 'NÃO'}</td>
                            <td>{(data.you_have_custom_consuming_sug_dr_cak_bis_stuf_dess_2_11 == "1") ? 'SIM' : 'NÃO'}</td>
                            <td>{(data.in_the_prep_your_meals_great_qt_oils_used_fats_su_salt_2_11 == "1") ? 'SIM' : 'NÃO'}</td>
                            <td>{(data.do_you_include_water_intake_in_your_daily_routine_2_11 == "1") ? 'SIM' : 'NÃO'}</td>
                            <td>{(data.do_you_currently_smoke_any_tobacco_products_2_11 == "1") ? 'SIM' : 'NÃO'}</td>
                            <td>{(data.would_you_like_to_quit_smoking_2_11 == "1") ? 'SIM' : 'NÃO'}</td>
                            <td>{(data.are_you_a_former_smoker_2_11 == "1") ? 'SIM' : 'NÃO'}</td>
                            <td>{(data.do_you_drink_alcohol_2_11 == "1") ? 'SIM' : 'NÃO'}</td>
                            <td>{(data.have_you_ever_felt_the_need_to_reduce_or_of_alcohol_2_11 == "1") ? 'SIM' : 'NÃO'}</td>
                            <td>{(data.has_anyone_ever_criticized_you_for_drinking_2_11 == "1") ? 'SIM' : 'NÃO'}</td>
                            <td>{(data.feel_guilty_for_drinking_2_11 == "1") ? 'SIM' : 'NÃO'}</td>
                            <td>{(data.do_you_usually_drink_in_the_morning_2_11 == "1") ? 'SIM' : 'NÃO'}</td>
